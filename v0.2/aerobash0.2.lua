...
ukeshotdist = 5 -- How close to one of Uke's guide dots Tori has to be in order for Uke to shoot.
...
function kdown(k)
	if editkeys == 0 then
		if k == keys.reloadscript then run_cmd("loadscript aerobash0.2.lua") end
		...
		if k == keys.fire_l and flags.fire == 0 then fire_l(0) end
		if k == keys.fire_r and flags.fire == 0 then fire_r(0) end
...
function ukemissiles()
	local pa = get_body_info(1,3).pos
	local pb = get_body_info(1,2).pos
	if flags.ul ~= 0 then
		local pm = get_body_info(1,missilenumbers[8-flags.ul]).pos
		for i = 1,guide.dots do
			...
			local thisdot = { x = pm.x+i*(pb.x-pa.x)*guide.length/guide.dots/distbetween(pb,pa), y = pm.y+i*(pb.y-pa.y)*guide.length/guide.dots/distbetween(pb,pa), z = pm.z+i*(pb.z-pa.z)*guide.length/guide.dots/distbetween(pb,pa) }
			if distbetween(thisdot,get_body_info(0,1).pos) < ukeshotdist then
				
			end
		end
	end
	if flags.ur ~= 0 then
		local pm = get_body_info(1,missilenumbers[15-flags.ur]).pos
		for i = 1,guide.dots do
			set_color(guide.r,guide.g,guide.b,guide.a)
			draw_sphere_m(pm.x+i*(pb.x-pa.x)*guide.length/guide.dots/distbetween(pb,pa),pm.y+i*(pb.y-pa.y)*guide.length/guide.dots/distbetween(pb,pa),pm.z+i*(pb.z-pa.z)*guide.length/guide.dots/distbetween(pb,pa),guide.dotradius,0/distbetween(pb,pa))
			local thisdot = { x = pm.x+i*(pb.x-pa.x)*guide.length/guide.dots/distbetween(pb,pa), y = pm.y+i*(pb.y-pa.y)*guide.length/guide.dots/distbetween(pb,pa), z = pm.z+i*(pb.z-pa.z)*guide.length/guide.dots/distbetween(pb,pa) }
...
function startgame()
	flags = {
		...
		l = 7, -- Tori's number of left-wing missiles left. Can be an integer from 0 to 7.
		r = 7, -- Tori's number of right-wing missiles left. Can be an integer from 0 to 7.
		ul = 7, -- Uke's number of left-wing missiles left. Can be an integer from 0 to 7.
		ur = 7, -- Uke's number of right-wing missiles left. Can be an integer from 0 to 7.
		fire = 0,
